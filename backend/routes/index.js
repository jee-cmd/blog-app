...
router.post("/updateBlog", upload.single('image'), async (req, res) => {
  try {
    let { token, blogId, title, desc, content } = req.body;
    let decoded = jwt.verify(token, secret);
    let user = await userModel.findOne({ _id: decoded.userId });
    ...
    // Verify ownership
    let existingBlog = await blogModel.findById(blogId);
    if (!existingBlog) {
      return res.json({ success: false, msg: "Blog not found" });
    }
    if (existingBlog.user.toString() !== user._id.toString()) {
      return res.json({ success: false, msg: "You are not authorized to edit this blog" });
    }

    let updateData = { title, desc, content };
    if (req.file) {
      updateData.image = req.file.filename;
    }

    let blog = await blogModel.findByIdAndUpdate(blogId, updateData, { new: true });
...
router.post("/deleteBlog", async (req, res) => {
  try {
    ...
    if (existingBlog.user.toString() !== user._id.toString()) {
      return res.json({ success: false, msg: "You are not authorized to delete this blog" });
    }
